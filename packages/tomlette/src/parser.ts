...
import { ParseOptions, ParseResult, TomletteParseError } from './types';
...
export function parse(content: string, options: ParseOptions = {}): ParseResult {
  ...
  try {
    ...
    throw new TomletteParseError(`Failed to parse content: ${errorMessage}`, 'auto');
  }
}
 ...
 */
export function parseFileSync(filePath: string, options: ParseOptions = {}): ParseResult {
  const { logger } = options;
